...
class UserAPI:
    ...
    async def register_account(self, user: UserCreate, session: AsyncSession = Depends(get_session)):
        existing_user = await UserController.get_user_by_email(session, user.email)
        if existing_user:
            raise HTTPException(status_code=400, detail="Email already registered")
        
        await UserController.create_user(session, user.username, user.firstname, user.email, user.password)
        return {"message": "User created successfully"}

    async def login_for_token(self, credentials: UserLogin, session: AsyncSession = Depends(get_session)):
        user = await UserController.get_user_by_email(session, credentials.email)
